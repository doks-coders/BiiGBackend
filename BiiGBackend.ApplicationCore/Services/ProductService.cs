...
namespace BiiGBackend.ApplicationCore.Services
{
	public class ProductService : IProductService
	{
		...
		public async Task<ResponseModal> FilterProducts(FilterProductsPaginationRequest request)
		{
			...
			predicate = predicate.And(u => u.ProductDetailsUploaded == true);
		...
		{
			var products = await _unitOfWork.Product.GetItems(u => u.ProductDetailsUploaded, includeProperties: "ProductPhotos,ProductBrand,ProductCategory");
		...
		{
			var product = await _unitOfWork.Product.GetItems(u => u.isRecentlyAdded == true, includeProperties: "ProductPhotos,ProductBrand,ProductCategory");
		...
		{
			var product = await _unitOfWork.Product.GetItems(u => u.isFeatured == true, includeProperties: "ProductPhotos,ProductBrand,ProductCategory");
			var response = product.GetProductResponse();
			return ResponseModal.Send(response);
		}
		...
		{
			var product = await _unitOfWork.Product.GetItem(u => u.Id == productId);
